...
const App = (() => {
  ...
  let domainData = {};
  let searchIndex = [];
  let bookmarks    = JSON.parse(localStorage.getItem('cbk_bookmarks') || '[]');
  let readSections = JSON.parse(localStorage.getItem('cbk_read')      || '{}');
  let ttsState = { active: false, paused: false, utterance: null, sectionIdx: 0, charIdx: 0 };
  let theme = localStorage.getItem('cbk_theme') || 'light';
  ...
  function applyTheme(t) {
    ...
    localStorage.setItem('cbk_theme', t);
  }
  ...
  function renderWelcome() {
    ...
    els.mainContent.innerHTML = `
      <div class="welcome-screen">
        <h1>CISSP CBK Reference</h1>
        <p class="subtitle">5th Edition &middot; Official (ISC)² CBK &middot; ${totalRead} sections completed</p>
    ...
    html.push(`
      <div class="content-area">
        <div class="chapter-header">
          ...
          <div class="chapter-meta">${data.sections.length} sections &middot; CBK 5th Edition</div>
        </div>
    `);
  ...
  function toggleBookmark(domainNum, sectionId, title) {
    ...
    localStorage.setItem('cbk_bookmarks', JSON.stringify(bookmarks));
    renderBookmarks();
    updateBookmarkCount();
  ...
  // ===== Progress =====
  const SECTION_COUNTS = { 1: 51, 2: 43, 3: 76, 4: 28, 5: 30, 6: 28, 7: 80, 8: 27 };
  ...
  function toggleRead(readKey, domainNum) {
    ...
    localStorage.setItem('cbk_read', JSON.stringify(readSections));
    renderTOC();
    renderProgress();
  }
