/* ===== CISSP Study Resources Reader App ===== */
...
const App = (() => {
  ...
  let domainData = {};
  let searchIndex = [];
  let bookmarks    = JSON.parse(localStorage.getItem('sr_bookmarks') || '[]');
  let readSections = JSON.parse(localStorage.getItem('sr_read')      || '{}');
  let ttsState = { active: false, paused: false, utterance: null, sectionIdx: 0, charIdx: 0 };
  let theme = localStorage.getItem('sr_theme') || 'light';
  ...
  function handleHash(hash) {
    ...
    if (match) navigateToDomain(parseInt(match[1]), match[2] || null);
  }
  ...
  function renderWelcome() {
    ...
    els.mainContent.innerHTML = `
      <div class="welcome-screen">
        <h1>CISSP Study Resources</h1>
        <p class="subtitle">2024 Exam Objectives &middot; jefferywmoore/CISSP-Study-Resources &middot; ${totalRead} sections completed</p>
        <div class="domain-cards">
          ${DOMAIN_LIST.map(d => {
            const readCount = Object.keys(readSections).filter(k => k.startsWith(`d${d.num}_`)).length;
            const progressPct = readCount > 0 ? Math.min(100, Math.round(readCount / SECTION_COUNTS[d.num] * 100)) : 0;
            return `
              <div class="domain-card" onclick="App.goToDomain(${d.num})">
                <div class="domain-num">Domain ${d.num}</div>
                <h3>${d.title}</h3>
                <div class="domain-stats">${readCount} of ${SECTION_COUNTS[d.num]} sections read</div>
    ...
    html.push(`
      <div class="content-area">
        <div class="chapter-header">
          ...
          <div class="chapter-meta">${data.sections.length} sections &middot; 2024 Exam Objectives</div>
        </div>
    `);
    ...
    return content.map(block => {
      ...
      if (block.type === 'figure') {
        ...
        const imgHtml = imgFile
          ? `<img src="/study-resources/figures/${imgFile}" alt="Figure ${escHtml(block.figNum)}" loading="lazy" class="book-figure-real-img">`
  ...
  function toggleTOCDomain(num) {
    if (tocExpanded[num]) {
      ...
      renderTOC();
      loadDomain(num).then(() => { if (tocExpanded[num]) renderTOC(); });
    }
  }
  ...
  function toggleBookmark(domainNum, sectionId, title) {
    ...
    else bookmarks.push({ id: sectionId, domain: domainNum, title, domainTitle: getDomainTitle(domainNum) });
  ...
  // ===== Progress =====
  const SECTION_COUNTS = { 1: 13, 2: 7, 3: 11, 4: 4, 5: 7, 6: 6, 7: 16, 8: 6 };

  function toggleRead(readKey, domainNum) {
    if (readSections[readKey]) delete readSections[readKey];
    else readSections[readKey] = Date.now();
    localStorage.setItem('sr_read', JSON.stringify(readSections));
    renderTOC();
    renderProgress();
  }
  ...
  function renderProgress() {
    ...
    els.progressPanel.innerHTML = DOMAIN_LIST.map(d => {
      const total = SECTION_COUNTS[d.num];
      ...
      return `
        <div style="padding:12px 16px; border-bottom:1px solid var(--rule)">
          ...
          <div class="progress-label" style="margin-top:4px">${read}/${total} sections · ${pct}%</div>
        </div>`;
    }).join('');
  }
  ...
  function handleSearch(query) {
    ...
    if (!query.trim()) { els.searchDropdown.classList.remove('open'); return; }
  ...
  function setupScrollSpy() {
    ...
    const observer = new IntersectionObserver(entries => {
      entries.forEach(e => {
        if (e.isIntersecting) updateActiveTOC(e.target.id.replace('sec-', ''));
      });
    }, { threshold: 0.3 });
  ...
  function updateActiveTOC(activeId) {
    document.querySelectorAll('.toc-section, .toc-subsection').forEach(el => el.classList.remove('active'));
    if (activeId) {
      const tocEl = document.getElementById(`toc-${activeId}`);
      if (tocEl) { tocEl.classList.add('active'); tocEl.scrollIntoView({ block: 'nearest' }); }
    }
  }
  ...
  function bindKeyboard(e) {
    ...
    switch (e.key) {
      ...
      case 'ArrowLeft': case 'p':
        if (currentDomain && domainData[currentDomain]) {
          scrollToSection(domainData[currentDomain].sections[Math.max(0, currentSectionIdx - 1)].id);
        }
        break;
  ...
  function createSelectionPopup() {
    ...
    el.id = 'cbk-selection-popup';
    el.innerHTML = `<button id="cbk-send-practice-btn" title="Send to Practice">📗 Send to Practice</button>`;
    document.body.appendChild(el);
    document.getElementById('cbk-send-practice-btn').addEventListener('click', sendSelectionToPractice);
    selectionPopup = el;
  }
  ...
  function handleTextSelection() {
    ...
    if (!text || text.length < 2 || text.length > 300) { hideSelectionPopup(); return; }
    if (!sel.anchorNode || !els.mainContent.contains(sel.anchorNode)) { hideSelectionPopup(); return; }
  ...
  async function sendSelectionToPractice() {
    ...
    const btn = document.getElementById('cbk-send-practice-btn');
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => { hideSelectionPopup(); btn.textContent = '📗 Send to Practice'; btn.disabled = false; }, 1200);
      } else {
  ...
  function bindEvents() {
    ...
    document.addEventListener('click', e => {
      if (!e.target.closest('.header-search')) els.searchDropdown && els.searchDropdown.classList.remove('open');
    });
  ...
  function getDomainTitle(num) {
    const d = DOMAIN_LIST.find(x => x.num === num);
    return d ? d.title : `Domain ${num}`;
  }
